...
static func lambda_attach_monitor_to_animator(uuid):
	...
	var monitor: MessageBox = Variables.messages[uuid]
...
static func lambda_update_monitor_messages(monitor_message: String, uuid):
	Variables.messages[uuid].display_message_immediately(monitor_message)
	
static func setup_monitor(uuid):
	# WIP
	...
	Queue.queue.append(Event.lambda().initialise(lambda_attach_monitor_to_animator, [uuid]))
...
static func display_question_and_options(Presenter, question_: String, options_: Array, uuid):
	Queue.queue.append(Event.settings().initialise({"message_box_position": [0,150]}))
