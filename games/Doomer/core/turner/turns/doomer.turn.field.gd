...
func start_betting_round_turns():
	...
	await doomer.scene.poker_board.lambdas.randomise_cards(doomer.board.all_cards)
	await doomer.scene.poker_board.lambdas.show_message("Lets see the hand..", false, _Doomer_Poker_Board_Events.MessageType.Log)
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.board.player_cards, _Doomer_Card.CardState.FacingUp, wait_for_each_card)
	await doomer.scene.poker_board.lambdas.show_message("Turning flop cards.", false, _Doomer_Poker_Board_Events.MessageType.Log)
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.board.flop_cards, _Doomer_Card.CardState.FacingUp, wait_for_each_card)
...
func flip_next_card_turns():
	...
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.board.next_field_card)
...
func flip_all_cards_down_turns():
	...
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.board.field_cards, _Doomer_Card.CardState.FacingDown, wait_for_each_card)
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.board.player_and_enemy_cards, _Doomer_Card.CardState.FacingDown, wait_for_each_card)


func show_enemy_hand_and_winner_decision():

	await doomer.scene.poker_board.lambdas.flip_cards(doomer.board.enemy_cards, _Doomer_Card.CardState.FacingUp, false)
	await doomer.scene.poker_board.lambdas.card_attack(doomer.board.field_cards, doomer.board.loser_opponent, 10)
	await doomer.scene.poker_board.lambdas.change_coins(100, doomer.board.winner_coin_box)
	await doomer.scene.poker_board.lambdas.demark_cards(doomer.board.all_cards, _Doomer_Card.MarkPointers.all_marks, false)
