...
func start_betting_round_turns():
	...
	await doomer.scene.poker_board.lambdas.randomise_cards(doomer.pointer.all_cards)
	await doomer.scene.poker_board.lambdas.show_message("Lets see the hand..", false, _Doomer_Poker_Board_Events.MessageType.Log)
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.pointer.player_cards, _Doomer_Card.CardState.FacingUp, wait_for_each_card)
	await doomer.scene.poker_board.lambdas.show_message("Turning flop cards.", false, _Doomer_Poker_Board_Events.MessageType.Log)
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.pointer.flop_cards, _Doomer_Card.CardState.FacingUp, wait_for_each_card)
...
func flip_next_card_turns():
	...
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.pointer.next_field_card)
...
func flip_all_cards_down_turns():
	...
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.pointer.field_cards, _Doomer_Card.CardState.FacingDown, wait_for_each_card)
	await doomer.scene.poker_board.lambdas.flip_cards(doomer.pointer.player_and_enemy_cards, _Doomer_Card.CardState.FacingDown, wait_for_each_card)


func show_enemy_hand_and_winner_decision():

	await doomer.scene.poker_board.lambdas.flip_cards(doomer.pointer.enemy_cards, _Doomer_Card.CardState.FacingUp, false)
	await doomer.scene.poker_board.lambdas.card_attack(doomer.pointer.field_cards, doomer.pointer.loser_opponent, 10)
	await doomer.scene.poker_board.lambdas.change_coins(100, doomer.pointer.winner_coin_box)
	await doomer.scene.poker_board.lambdas.demark_cards(doomer.pointer.all_cards, _Doomer_Card.MarkPointers.all_marks, false)
