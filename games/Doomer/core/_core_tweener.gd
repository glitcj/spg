...
static func _slide_in(tweenee : Node, duration := 0.5, vector = Vector2i(0, 1), base_position : Vector2 = Vector2.ZERO):
...
static func _slide_out(tweenee : Node, duration := 0.5, vector = Vector2i(0, 1)):
	...
	# CHANGE ZERO VECTOR TO BASE POSITION
	# tween.tween_callback(func(): tweenee.position = Vector2.ZERO)
