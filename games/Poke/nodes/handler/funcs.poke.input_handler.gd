extends Node
class_name _Poke_Handler

@onready var poke: _Poke = get_parent()
...
func _on_fold_action():
	poke.opponent_portrait.animation_player.play("Happy")
	poke.opponent_portrait.animation_player.queue("RESET")
